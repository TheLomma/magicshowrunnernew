...
var T = {
  de: {
    title: "Magic Showrunner", ver: "v5.5", save: "Speichern", load: "Laden", newPart: "Neuer Teil",
  ...
  en: {
    title: "Magic Showrunner", ver: "v5.5", save: "Save", load: "Load", newPart: "New Part",
  ...
  return (
    <Modal open={open} onClose={onClose} title={t.templates} th={th}>
      ...
      {tpls.map(function (tpl, i) {
        return (<div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 8, marginBottom: 4, background: th.inp }}>
          <div style={{ width: 8, height: 8, borderRadius: 4, background: tpl.color || COLORS[0], flexShrink: 0 }} />
          <div style={{ flex: 1 }}><div style={{ fontWeight: 600, fontSize: 13 }}>{tpl.title}</div><div style={{ fontSize: 11, color: th.sub }}>{fmt(tpl.duration)}</div></div>
  ...
  return (
    <Modal open={open} onClose={onClose} title={part ? t.edit : t.newPart} th={th}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
        {COLORS.map(function (c) { return <div key={c} onClick={function () { up("color", c); }} style={{ width: 28, height: 28, borderRadius: 8, background: c, cursor: "pointer", border: f.color === c ? "3px solid #fff" : "3px solid transparent" }} />; })}
      </div>
...
function SaveModal(props) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={t.save} th={th}>
      ...
      {saves.map(function (s) { return <div key={s.name} onClick={function () { setName(s.name); }} style={{ padding: "6px 10px", background: name === s.name ? th.acc + "22" : "transparent", borderRadius: 8, cursor: "pointer", fontSize: 13, color: th.text, marginBottom: 2 }}>{s.name} ({s.parts.length} {t.parts})</div>; })}
  ...
  return (
    <Modal open={open} onClose={onClose} title={t.load} th={th}>
      ...
      {saves.map(function (s) {
        return (<div key={s.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px", borderRadius: 8, marginBottom: 4, background: th.inp }}>
          <div><div style={{ fontWeight: 600, fontSize: 14 }}>{s.name}</div><div style={{ fontSize: 11, color: th.sub }}>{s.parts.length} {t.parts}</div></div>
...
function SettingsModal(props) {
  var open = props.open, onClose = props.onClose, cfg = props.cfg, setCfg = props.setCfg, t = props.t, th = props.th, onApplyCustom = props.onApplyCustom;
  ...
  if (tab === "design") {
    ...
    content = (<div>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
        {Object.keys(SOUNDS).map(function (k) { return <button key={k} onClick={function () { upCfg("beepSound", k); doBeep(cfg.volume, null, null, k); }} style={{ padding: "6px 12px", borderRadius: 8, border: cfg.beepSound === k ? "2px solid " + th.acc : "2px solid transparent", background: cfg.beepSound === k ? th.acc + "22" : th.inp, color: th.text, cursor: "pointer", fontSize: 12 }}>{SOUNDS[k].label}</button>; })}
      </div>
    ...
    content = (<div>
      <label style={{ fontSize: 12, color: th.sub }}>{cfg.lang === "de" ? "Anzeigegröße im Show-Modus" : "Display size in Show Mode"}</label>
      <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
        {["XXL", "XL", "M", "S"].map(function (sz) { return <button key={sz} onClick={function () { upCfg("performSize", sz); }} style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: cfg.performSize === sz ? "2px solid " + th.acc : "2px solid transparent", background: cfg.performSize === sz ? th.acc + "22" : th.inp, color: th.text, cursor: "pointer", fontWeight: cfg.performSize === sz ? 700 : 400, fontSize: 14 }}>{sz}</button>; })}
      </div>
    ...
    content = (<div style={{ display: "flex", gap: 8 }}>
      {["de", "en"].map(function (l) { return <button key={l} onClick={function () { upCfg("lang", l); }} style={{ flex: 1, padding: 12, borderRadius: 10, border: cfg.lang === l ? "2px solid " + th.acc : "2px solid transparent", background: cfg.lang === l ? th.acc + "22" : "transparent", color: th.text, cursor: "pointer", fontSize: 16 }}>{l === "de" ? "DE Deutsch" : "EN English"}</button>; })}
    </div>);
  }
  return (
...
function PerformMode(props) {
    ...
    var iv = setInterval(function () {
      setElapsed(function (e) {
        ...
        if (ne >= dur) {
          ...
          if (idx < parts.length - 1) { setIdx(function (i) { return i + 1; }); introRef.current = {}; preAnnRef.current = {}; return 0; }
  ...
  useEffect(function () {
    ...
    if (cur && cur.intro && !introRef.current[idx]) { introRef.current[idx] = true; doSpeak(cur.intro, cfg.ttsRate, cfg.ttsPitch, cfg.ttsVoice); }
  }, [idx, cdRunning, done]);
  ...
  if (blackout) {
    return (<div style={{ position: "fixed", inset: 0, background: "#000", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 999, cursor: "pointer" }} onClick={function () { setBlackout(false); }}>
  ...
  if (cdRunning) {
    return (<div style={{ position: "fixed", inset: 0, background: pt.bg, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
      <div style={{ textAlign: "center" }}><div style={{ fontSize: 120, fontWeight: 800, color: pt.timer }}>{cdVal}</div><div style={{ fontSize: 18, color: pt.text, opacity: 0.6 }}>{t.countdown}...</div></div>
    </div>);
  }
  if (done) {
    return (<div style={{ position: "fixed", inset: 0, background: pt.bg, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
      <div style={{ textAlign: "center" }}><div style={{ fontSize: 48, fontWeight: 800, color: pt.text }}>🎉</div><div style={{ fontSize: 24, color: pt.text, marginBottom: 24 }}>{cfg.lang === "de" ? "Show beendet!" : "Show finished!"}</div>
      <button onClick={onExit} style={{ padding: "12px 32px", borderRadius: 12, border: "none", background: pt.bar, color: "#fff", fontSize: 18, fontWeight: 700, cursor: "pointer" }}>OK</button></div>
    </div>);
  }
  if (confirmStop) {
  ...
  var handleSeek = function (e) {
    ...
    setElapsed(Math.floor((x / rect.width) * dur));
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: bgStyle, display: "flex", flexDirection: "column", zIndex: 999, transition: useColorTrans ? "background 1.5s ease" : "none" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 24px" }}>
        ...
        {showNotes && cur.notes && (<div style={{ marginTop: 16, padding: 12, background: pt.barBg, borderRadius: 10, maxWidth: 400, width: "100%", fontSize: 14, color: pt.text }}>{cur.notes}</div>)}
      </div>
      {showSetlist && (<div style={{ position: "absolute", right: 0, top: 50, bottom: 0, width: 200, background: pt.bg, borderLeft: "1px solid " + pt.barBg, padding: 12, overflowY: "auto" }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: pt.text, marginBottom: 8 }}>{t.setlist}</div>
        {parts.map(function (p, i) {
          return (<div key={p.id} onClick={function () { setIdx(i); setElapsed(0); preAnnRef.current = {}; introRef.current = {}; }} style={{ padding: "6px 8px", borderRadius: 6, marginBottom: 2, background: i === idx ? pt.bar : "transparent", color: i === idx ? "#fff" : pt.text, cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", gap: 6 }}>
...
function exportCSV(parts) {
  ...
  var rows = parts.map(function (p) { return [p.title, p.duration, p.intro, p.preAnn, p.preAnnText, p.notes, p.color].map(function (v) { return '"' + String(v || "").replace(/"/g, '""') + '"'; }).join(","); }).join("\n");
  var blob = new Blob([header + rows], { type: "text/csv" }); var a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "showrunner.csv"; a.click();
}

function exportTXT(parts) {
  var txt = parts.map(function (p, i) { return (i + 1) + ". " + p.title + " (" + fmt(p.duration) + ")"; }).join("\n");
  var blob = new Blob([txt], { type: "text/plain" }); var a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "setlist.txt"; a.click();
}
...
export default function App() {
  ...
  var _parts = useState(DEMO); var parts = _parts[0], setParts = _parts[1];
  ...
  var savePart = function (p) {
    setParts(function (ps) { var i = ps.findIndex(function (x) { return x.id === p.id; }); if (i >= 0) { var n = ps.slice(); n[i] = p; return n; } return ps.concat([p]); });
  };
  ...
  var handleDrop = function (i) {
    ...
    setParts(function (ps) { var n = ps.slice(); var item = n.splice(dragIdx, 1)[0]; n.splice(i, 0, item); return n; });
    setDragIdx(null);
  };
      ...
      try {
        ...
        for (var i = 1; i < lines.length; i++) {
          ...
          if (vals && vals.length >= 2) {
            ...
            imported.push({ id: uid(), title: clean(vals[0]), duration: parseInt(clean(vals[1])) || 120, intro: clean(vals[2] || ""), preAnn: parseInt(clean(vals[3] || "")) || 10, preAnnText: clean(vals[4] || ""), notes: clean(vals[5] || ""), color: clean(vals[6] || "") || COLORS[0] });
          }
        }
        if (imported.length > 0) { setParts(imported); setToast("CSV OK – " + imported.length + " " + t.parts); }
      } catch (err) { setToast("CSV Error"); }
    };
    reader.readAsText(file); e.target.value = "";
  };

  var startShow = function (inBlackout) { setStartBlackout(inBlackout || false); setPerf(true); };

  if (perf) {
    return (<PerformMode parts={parts} cfg={cfg} onExit={function () { setPerf(false); setStartBlackout(false); }} startInBlackout={startBlackout} />);
  }

  var bs = function (extra) { return Object.assign({ padding: "8px 16px", borderRadius: 10, border: "none", cursor: "pointer", fontWeight: 600, fontSize: 13 }, extra || {}); };
  ...
  return (
    <div style={{ minHeight: "100vh", background: th.bg, color: th.text, fontFamily: cfg.fontFamily === "System" ? "-apple-system, BlinkMacSystemFont, sans-serif" : cfg.fontFamily, fontSize: cfg.fontSize, transition: "background 0.3s" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          {parts.map(function (p, i) {
            return (<div key={p.id} draggable onDragStart={function () { handleDragStart(i); }} onDragOver={function (e) { e.preventDefault(); }} onDrop={function () { handleDrop(i); }}
              style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: th.card, borderRadius: 10, marginBottom: 6, border: "1px solid " + th.brd, cursor: "grab", transition: cfg.animations ? "transform 0.15s" : "none" }}>
              <div style={{ width: 6, height: 32, borderRadius: 3, background: p.color, flexShrink: 0 }} />
              <div style={{ flex: 1 }}><div style={{ fontWeight: 600, fontSize: 14 }}>{p.title}</div><div style={{ fontSize: 11, color: th.sub }}>{fmt(p.duration)}</div></div>
      ...
      <SettingsModal open={settOpen} onClose={function () { setSettOpen(false); }} cfg={cfg} setCfg={setCfg} t={t} th={th} onApplyCustom={handleApplyCustom} />
